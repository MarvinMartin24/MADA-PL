...
class MADA(pl.LightningModule):
    ...
    def __init__(self, cfg, mode, dataModule=None):
        ...
        if mode == 'Train':
            ...
            self.train_accuracy_domain_tgts = {}
            for tqt in self.cfg['input']['dataset']['tgts']:
                train_accuracy_domain_tgts[f"train_accuracy_domain_tgt_{tqt}"] = pl.metrics.Accuracy()
            
            # Init Train metrics for source and targets
            self.val_accuracy_classif =  pl.metrics.Accuracy()
            
            # Init test metrics for source and targets
            self.test_accuracy_classif_src =  pl.metrics.Accuracy()
            self.test_accuracy_classif_tqts = {}
            for tqt in self.cfg['input']['dataset']['tgts']:
                test_accuracy_classif_tqts[f"test_accuracy_classif_tqt_{tqt}"] = pl.metrics.Accuracy()
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("Train/acc_classif", self.train_accuracy_class, prog_bar=True)
    ...
    def training_epoch_end(self, outs):
        self.log("Train/acc_classif_epoch", self.train_accuracy_class.compute(), prog_bar=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss_src = self.criterion_classif(class_logit_src, ys)
        ...
        for (xt, yt) in batch_tqts:
            ...
            loss_tqt = self.criterion_classif(class_logit_tqt, yt)
